...
const api=async(p,o={})=>{try{const r=await fetch(`${API}${p}`,o);const j=await r.json().catch(()=>({}));if(!r.ok){const e=new Error((r.status===404&&!API)?'Inference service not connected. Set VITE_API_URL to the SpineMuscle inference backend.':(j.detail||j.error||`Inference service error (${r.status})`));e.status=r.status;throw e}return j}catch(e){if(e instanceof TypeError){const x=new Error('Cannot reach the SpineMuscle inference service. Check VITE_API_URL and that the backend is running.');x.status=0;throw x}throw e}};
  ...
  useEffect(()=>{
    ...
    const tick=async()=>{
      try{
        ...
        setStatus(s);
        setErr('');
        if(s?.stage==='complete'||s?.stage==='blocked'){
          stop=true;
        ...
        if(stop)return;
        if(e.status===404){
          const msg='Analysis session was interrupted or expired on the inference service. Please restart this study.';
          setErr(msg);
          setStatus({stage:'blocked',progress:100,qc_decision:'INTERRUPTED',result:{pvmq:null,pvmq_issued:false,qc_reason:msg}});
          stop=true;
          if(id)clearInterval(id);
        }else{
          setErr(e.message);
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {(stage==='complete'||stage==='blocked')&&<section className="panel report">
        <div className="panelHead"><div><p className="eyebrow">Research measurement report</p><h3>{stage==='complete'?'Analysis complete':'Study blocked during validation'}</h3></div><Badge tone={stage==='complete'?'good':'warn'}>{status?.qc_decision||'REVIEW'}</Badge></div>
